...
public abstract class MDWGenericBaseCommonDao<T, PK extends Serializable>
		implements IGenericBaseCommonDao {
	/**
	 * 初始化Log4j的一个实例
	 */
	private static final Logger logger = Logger.getLogger(MDWGenericBaseCommonDao.class);
	/**
	 * 注入一个sessionFactory属性,并注入到父类(HibernateDaoSupport)
	 * **/
	@Autowired
	@Qualifier("sessionFactoryMdw")
	private SessionFactory sessionFactoryMdw;

	public Session getSession() {
		// 事务必须是开启的(Required)，否则获取不到(待修改)
		return sessionFactoryMdw.getCurrentSession();
	}
	...
	private <T> void getProperty(Class entityName) {
		ClassMetadata cm = sessionFactoryMdw.getClassMetadata(entityName);
...
//		}
	...
	public DataGridReturn getDataGridReturn(final CriteriaQuery cq,
		...
		int allCounts = ((Long) criteria.setProjection(
				Projections.rowCount()).uniqueResult()).intValue();
//		if(projection != null) {
//			allCounts = ((Long) criteria.setProjection(
//					Projections.rowCount()).uniqueResult()).intValue();
//		}
		criteria.setProjection(projection);
		if (projection == null) {
	...
	@Autowired
	@Qualifier("mdwjdbcTemplate")
	private JdbcTemplate jdbcTemplate;

	@Autowired
	@Qualifier("mdwnamedParameterJdbcTemplate")
